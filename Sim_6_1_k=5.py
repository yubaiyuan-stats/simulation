...
for ii in range(repeat):
  ...
  b = np.array([1,1.2,1.4,1.6,1.8,2])
  for j in range(3):
...
##generate the underlying latent confounder U     
    ...
    #np.corrcoef(np.concatenate((treatment,a6.reshape(n,1)),axis=1).transpose())
...
#generate X, M ,and Y      
    ...
    coef_M = rvs(dim=5)
    M_whole = 0.5*Bias[:,0:5] + np.ones(5)*treatment_whole + 0.5*np.matmul(X_whole[:,0:5],coef_M[:,0:20]) + 1*noise_M
    true_M_whole = 0.5*Bias[:,0:5] + np.ones(5)*treatment_whole + 0.5*np.matmul(X_whole[:,0:5],coef_M[:,0:20])
    
    
    noise_Y = np.random.multivariate_normal([0], [[0.3]], N)
    Y_whole = 1*treatment_whole + (1.5*Bias[:,5]).reshape(N,1) + np.matmul(M_whole,0.5*np.ones(5)).reshape(N,1) + 0.3*np.matmul(X_whole[:,5:],[1,1]).reshape(N,1) + 1*noise_Y
    true_Y_whole = 1*treatment_whole + 1.5*Bias[:,5].reshape(N,1) + np.matmul(M_whole,0.5*np.ones(5)).reshape(N,1) + 0.3*np.matmul(X_whole[:,5:],[1,1]).reshape(N,1)
...
##set dimension of surrogate confounder as k = 3    
    
    k = 3
    U_scale = 10 #LA.norm(U)/k 
...
## update the models for treatment T 
...
##calculate the loss from model fitting            
            ...
            reg_Uhat = LinearRegression(fit_intercept=False).fit(Uhat,U[train_ID].reshape(n,1))
            corr_track.append(np.corrcoef(np.concatenate((reg_Uhat.predict(Uhat),U[train_ID].reshape(n,1)),axis=1).transpose())[0,1])    
            ...
            if total_loss < loss_ind:
                ...
                #h_PCA_MSE = ((LA.norm(reg_Y.predict(X_Y) - true_Y)**2)/n)**0.5
...
##make the out-sample prediction for outcome based on random forest  
    ...
    np.corrcoef(np.concatenate((Y_bias.reshape(n,1),U[train_ID].reshape(n,1)),axis=1).transpose()) 
    np.corrcoef(np.concatenate((Y_bias_pred.reshape(n1,1),U[test_ID].reshape(n1,1)),axis=1).transpose())           
...
########proposed method using autoencoder as latent confounding model   
    ...
    k  = 6
    U_scale = 10 # LA.norm(U)/2
       
    pca = PCA(n_components=6)
     # #pca.fit(M - reg_M.predict(X_M_r))
    pca.fit(np.concatenate((Y,M),axis=1))
    ...
    gamma_T = 0.1*np.ones(k).reshape(k,1)
     #gamma_T = np.array([0.176,0.362]).reshape(2,1)
    T_int = 0
...
###the dimension of surrogate confounder is 24  
    ...
    x1 = layers.Dense(12, activation='selu')(x1)
     #x1 = layers.Dense(16, activation='selu')(x1)
     #x1 = layers.Dense(8, activation='selu')(x1)
     #x1 = layers.Dense(32, activation='selu')(x1)
     #x1 = layers.Dense(16, activation='selu')(x1)
    x3 = layers.Dense(20, activation='selu', activity_regularizer = UncorrelatedFeaturesConstraint_target(20,treatment.reshape(n,), weightage = 20))(x1)
     #x1 = layers.Dense(4, activation='selu')(x1)
    
    #input_U_2 = keras.Input(shape=(6,))
    #x2 = layers.Dense(6, activation='tanh')(input_U_2)
    #x2 = layers.Dense(4, activation='selu')(x2)
     #x2 = layers.Dense(8, activation='selu')(x2)
    #x2 = layers.Dense(4, activation='selu')(x2)
     #x2 = layers.Dense(4, activation='selu',activity_regularizer = correlatedFeaturesConstraint_target(4, target = treatment.reshape(n,), weightage = 0.4))(x2)
     #x2 = layers.Dense(4, activation='selu')(x2)
    
    #encoded = layers.concatenate([x2,x1])
    ...
    #y = layers.Dense(12, activation='selu')(y)
     #y = layers.Dense(64, activation='selu')(y)
     #y = layers.Dense(128, activation='selu')(y)
     #y = layers.Dense(10, activation='selu',kernel_constraint=UnitNorm(axis=1))(y)
    ...
    forest_treatment[j,ii] = np.mean(abs(pred - 3.5))
   
    #forest_MSE[ii] = ((LA.norm(est.models_y[0][0].predict(X = covariate).reshape(n,1) - true_Y)**2)/n)**0.5
   ...
   ####X learner method
    ...
    X_treatment[j,ii] = np.mean(abs(X_te - 3.5))
    ...
    LR_T_bias[j,ii] = np.mean(abs(T_M - np.ones([1,5])))     
    LR_Y_bias[j,ii] = np.mean(abs(beta_Y_M - 0.5*np.ones([5,1]))) 
...
###output the results in Table 1 under k = 5
## the true treatment effect is 3,5, true mediation effect is 2.5  
np.mean(abs(PCA_treatment-3.5),1)
np.std(abs(PCA_treatment-3.5),1)
np.mean(abs(PCA_treatment_m-2.5),1)
np.std(abs(PCA_treatment_m-2.5),1)

np.mean(abs(mse_pca),1)
np.std(abs(mse_pca),1)

np.mean(abs(autoencoder_treatment-3.5),1)
np.std(abs(autoencoder_treatment-3.5),1)
np.mean(abs(autoencoder_treatment_m-2.5),1)
np.std(abs(autoencoder_treatment_m-2.5),1)

np.mean(abs(mse_auto),1)
np.std(abs(mse_auto),1)

np.mean(abs(LR_treatment-3.5),1)
np.std(abs(LR_treatment-3.5),1)
np.mean(abs(LR_treatment_m-2.5),1)
np.std(abs(LR_treatment_m-2.5),1)
...
for ii in range(repeat):
  ...
  for j in range(3):
    ...
    Bias= b*U
    Bias = (Bias - np.mean(Bias,axis=0))/np.std(Bias,axis = 0)
    np.corrcoef(np.concatenate((Bias,U.reshape(N,1)),axis=1).transpose())
...
########estimate causal effects and mediation effects via HIMA
    ...
    pred_Y_hima = np.matmul(test_M,hima_para[1,:].reshape(5,1)) + test_treatment*(hima_para[2,0] - np.sum(hima_para[3,:]))
    hima_MSE[j,ii] = (sum((pred_Y_hima.reshape(n1,1) - test_Y.reshape(n1,1))**2)/n1)**0.5     
     
 
np.mean(abs(hima_treatment-3.5),1)
np.std(abs(hima_treatment-3.5),1)
np.mean(abs(hima_treatment_m-2.5),1)
np.std(abs(hima_treatment_m-2.5),1)

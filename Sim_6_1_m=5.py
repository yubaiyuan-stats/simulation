...
from src.DataGene_Linear_k_5 import *
import numpy as np
...
#fix the random seed for each run   
  ...
  for j in range(3):
    ...
    X_whole, Y_whole, treatment_whole, M_whole = DataGene_Linear_k_5(N)
...
#split data into training and testing set randomly   
    ...
    Prop_FM(X= X, Y = Y, M = M, treatment = treatment,test_X=test_X,test_Y=test_Y,test_M=test_M,test_treatment=test_treatment,n=n,n1 = n1,m = m,method ='linear',latent_dim = 3)
    ...
    Prop_AE(X, Y, M, treatment,test_X,test_Y,test_M,test_treatment,Y_int,beta_Y_ini,beta_M_ini,beta_Y_M_ini,beta_Y_T_ini,T_M_ini,M_int,n,n1,m,method ='linear',latent_dim = 24,ini_dim = 6)
...
##LSEM method
 ...
 ###output the results in Table 1 under m = 5
 ## the true treatment effect is 3.5, true mediation effect is 2.5 
#Prop FM
np.mean(abs(PCA_treatment-3.5),1)  # value: 0.54, 0.21, 0.12
np.std(abs(PCA_treatment-3.5),1)   # value: 0.48, 0.17, 0.09
np.mean(abs(PCA_treatment_m-2.5),1)  # value: 0.68, 0.26, 0.24
np.std(abs(PCA_treatment_m-2.5),1)   # value: 0.60, 0.19, 0.18

np.mean(abs(mse_pca),1)  # value: 1.00, 0.95, 0.96
np.std(abs(mse_pca),1)   # value: 0.10, 0.03, 0.04

#Prop AE
np.mean(abs(autoencoder_treatment-3.5),1) # value: 1.10, 0.81, 0.75
np.std(abs(autoencoder_treatment-3.5),1)  # value: 0.27, 0.20, 0.20
np.mean(abs(autoencoder_treatment_m-2.5),1)  # value: 0.80, 0.32, 0.21
np.std(abs(autoencoder_treatment_m-2.5),1)   # value: 0.62, 0.19, 0.16

np.mean(abs(mse_auto),1)  # value: 0.99, 0.96, 0.94
np.std(abs(mse_auto),1)   # value: 0.10, 0.03, 0.04

#LSEM
np.mean(abs(LR_treatment-3.5),1)  # value: 1.19, 1.18, 1.17
np.std(abs(LR_treatment-3.5),1)   # value: 0.23, 0.14, 0.06
np.mean(abs(LR_treatment_m-2.5),1)  # value: 2.76, 2.73, 2.73
np.std(abs(LR_treatment_m-2.5),1)   # value: 0.27, 0.13, 0.07

np.mean(abs(mse_lr),1)  # value: 1.09, 1.06, 1.04
np.std(abs(mse_lr),1)   # value: 0.11, 0.03, 0.03

#Causal Forest
np.mean(forest_treatment,1)  # value: 3.77, 4.09, 4.16
np.std(forest_treatment,1)   # value: 0.49, 0.17, 0.11
np.mean(mse_forest,1)  # value: 1.46, 1.22, 1.15
np.std(mse_forest,1)   # value: 0.12, 0.06, 0.04

#XLearner
np.mean(X_treatment,1)  # value: 3.33, 3.71, 3.76
np.std(X_treatment,1)   # value: 0.37, 0.14, 0.07
np.mean(mse_X,1)  # value: 1.49, 1.24, 1.17
np.std(mse_X,1)   # value: 0.19, 0.06, 0.05
...
#HIMA      
np.mean(abs(hima_treatment-3.5),1)  # value: 2.42, 2.31, 2.34
np.std(abs(hima_treatment-3.5),1)   # value: 0.52, 0.19, 0.12
np.mean(abs(hima_treatment_m-2.5),1)  # value: 3.59, 3.49, 3.51
np.std(abs(hima_treatment_m-2.5),1)   # value: 0.47, 0.19, 0.10
np.mean(abs(hima_MSE),1)  # value: 1.21, 1.15, 1.15
np.std(abs(hima_MSE),1)   # value: 0.11, 0.05, 0.04

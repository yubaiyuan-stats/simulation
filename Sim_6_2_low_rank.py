...
autoencoder_MSE = np.zeros(repeat) 
...
LR_MSE = np.zeros(repeat) 
LR_P = np.zeros(repeat) 
LR_T_bias = np.zeros(repeat)
LR_Y_bias = np.zeros(repeat)


forest_treatment = np.zeros(repeat)
forest_MSE = np.zeros(repeat) 
forest_P = np.zeros(repeat)
mse_forest = np.zeros(repeat)

X_treatment = np.zeros(repeat)
X_MSE = np.zeros(repeat) 
X_P = np.zeros(repeat)
...
for ii in range(repeat):
    ...
    np.random.seed(ii)  
## for settings with different sample sizes, change N = 800, N = 2000
...
#generate T, M, Y   
    ...
    #true_Loss_Y = Y - true_Y
    np.corrcoef(np.concatenate((Y_whole,U.reshape(N,1)),axis=1).transpose())
    ...
    reg_U = LinearRegression(fit_intercept=False).fit(M, U[train_ID])
    np.corrcoef(np.concatenate((reg_U.predict(M).reshape(n,1),U[train_ID].reshape(n,1)),axis=1).transpose())   
    ...
    np.sum(beta_Y_M.reshape(1,5)*T_M)
...
#########proposed method using matrix factorization as latent confounding model  
    ...
    #Loss_M_T = np.concatenate((treatment - sigmoid(np.matmul(U_begin,gamma_T)+T_int),Loss_Y,Loss_M),axis=1)
    ...
    while iter<=200:
            ...
            for kk in range(k):
                ...
                #grad_corr = corr_grad(Loss_M_Y,gamma_U_Y)    
                #grad_U_k[:,kk] = (- np.sum(2*gamma_M[kk,:]*Loss_M,1).reshape(n,1) + lambda_1*grad_corr.reshape(n,1) + lambda_2*(-gamma_T[kk,0]*(treatment - P_est)).reshape(n,1)).reshape(n,)
                #grad_U_k[:,kk] = ( - 2*gamma_Y[kk,:]*Loss_Y  - np.sum(2*gamma_M[kk,:]*Loss_M,1).reshape(n,1) + lambda_1*grad_corr.reshape(n,1) + lambda_2*(-gamma_T[kk,0]*(treatment - P_est) - gamma_T[kk,0]*(1+np.log(P_est))*(P_est - P_est**2)).reshape(n,1)).reshape(n,)
...
#####proposed method using autoencoder as latent confounding model   
    ...
    gamma_Y = 0
     #res_Y =  Loss_Y
     #Loss_M = M - (M_int + np.matmul(X[:,0:4],beta_M.transpose()).reshape(n,4) + T_M*treatment + np.matmul(U_begin,gamma_M))
    ...
    while (total_loss_1 - total_loss> 1 and iter<=6) or iter <=2: 
         ...
         ######
            ...
            #weights_set = autoencoder.get_weights()  
            ...
            rf_train = np.concatenate((treatment,M,X),axis=1)
            rf_label = T_M_Y_hat[:,1].reshape(n,1)
            ...
            if total_loss < loss_ind:
                ...
                loss_ind  = total_loss
                #corr_ind  = corr_track[-1]
                mse_auto_3 = min(predict_track)
   ...
   #  ##################X learner method
    ...
    X_P[ii] = np.corrcoef(np.concatenate((prop_score,P),axis=1).transpose())[0,1] 
    ...
    LR_T_bias[ii] = np.mean(abs(T_M - np.ones([1,5])))     
    LR_Y_bias[ii] = np.mean(abs(beta_Y_M - 0.5*np.ones([5,1]))) 
    ...
    LR_P[ii] = np.corrcoef(np.concatenate((reg_LR.predict(X_Y_U),P),axis=1).transpose())[0,1]
    ...
    pca.fit(res_M)
    U_M = pca.fit_transform(res_M) #+ 2*np.random.randn(n, k) 
    
    reg_Uini = LinearRegression().fit(U_M,P)
    np.corrcoef(np.concatenate((reg_Uini.predict(U_M),Bias[train_ID,5].reshape(n,1)),axis=1).transpose())[0,1]
    np.corrcoef(np.concatenate((Y_bias,Bias[train_ID,5].reshape(n,1)),axis=1).transpose())[0,1]
    
    reg_M_res = LinearRegression().fit(res_M,Bias[train_ID,5].reshape(n,1))
    np.corrcoef(np.concatenate((reg_M_res.predict(res_M),Bias[train_ID,5].reshape(n,1)),axis=1).transpose())
 
    reg_M_res = LinearRegression().fit(res_M,treatment.reshape(n,1))
    np.corrcoef(np.concatenate((reg_M_res.predict(res_M),treatment.reshape(n,1)),axis=1).transpose())
